...
function getProviderVisitor(
  user: any,
  room: Room,
  participants: Array<RemoteParticipant>,
  visit: TelehealthVisit
): any {
  return user.role == 'providervisitor'
    ? room!.localParticipant
...
function getChatUsers(
  ...
  participants: Array<RemoteParticipant>,
  visit: TelehealthVisit,
  patientVisitorName: string,
  providerVisitorName: string
): ChatUser[] {
  ...
  const providerVisitor = getProviderVisitor(
    user,
    room,
    participants as RemoteParticipant[],
    visit
